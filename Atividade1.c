...
// Função para buscar um aluno pelo nome na lista
int buscaAlunoPorNome(ListaAlunos* listaAlunos, char* nome) {
    for (int i = 0; i < listaAlunos->numElem; i++) {
        if (strcmp(listaAlunos->alunos[i].nome, nome) == 0) {
            return i; // Retorna a posição do aluno na lista
        }
    }
    return -1; // Retorna -1 se o aluno não for encontrado
}

// Função para buscar um aluno pela matrícula na lista
...
// Função para retornar o enésimo aluno na lista
Aluno retornaEnesimoAluno(ListaAlunos* listaAlunos, int n) {
    if (n >= 0 && n < listaAlunos->numElem) {
        return listaAlunos->alunos[n];
    } else {
        // Retorna um aluno com valores nulos se a posição for inválida
        Aluno alunoVazio;
        alunoVazio.matricula = 0;
        alunoVazio.nota = 0.0;
        strcpy(alunoVazio.nome, "");
        return alunoVazio;
    }
}
...
int main() {
    ...
    int posicaoMatricula = buscaAlunoPorMatricula(&listaAlunos, 22222222);
    ...
    int n = 1; // Retorna o segundo aluno da lista (índice 1)
    Aluno enesimoAluno = retornaEnesimoAluno(&listaAlunos, n);
    if (enesimoAluno.matricula != 0) {
        printf("Enésimo aluno na lista (n=%d):\n", n);
        printf("Nome: %s\n", enesimoAluno.nome);
        printf("Matricula: %d\n", enesimoAluno.matricula);
        printf("Nota: %.1f\n", enesimoAluno.nota);
    } else {
        printf("Posição inválida para o enésimo aluno.\n");
    }
